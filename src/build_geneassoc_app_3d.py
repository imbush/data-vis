...
// Gene analogue of "similar cells". Each SHOWN gene (geneActive[]) gets a
// Gaussian-kernel effective neighbour count among the OTHER shown genes, using
// the current 3D gene biplot coords (gene_x/gene_y/gene_z). Bandwidth h^2 =
// median squared pairwise distance among up to ~400 shown genes (median
// heuristic). Colours are built as a PLAIN Array (typed arrays coerce colour
// strings to NaN).
function colorBySimilarGenes() {
  let shown = (typeof visibleGeneIdx === 'function') ? visibleGeneIdx() : null;
  if (!shown || shown.length < 2) { shown = []; for (let j=0;j<N_GENES;j++) shown.push(j); }
  const G = shown.length;
  const inShown = new Uint8Array(N_GENES);
  ...
  const dens = new Float64Array(N_GENES);
  ...
  const palette = valuesToViridisArr(vals);
  let vi = 0;
  const colors = [];
  for (let j = 0; j < N_GENES; j++) colors.push(inShown[j] ? palette[vi++] : '#e6e6e6');
  Plotly.restyle(genePlot, {'marker.color': [colors]}, [POINTS_TRACE]);
  let lo = Infinity, hi = -Infinity;
