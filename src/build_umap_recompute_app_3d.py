...
// Embedding coords come from cell_score[i] (length-3 [UMAP1,2,3]; missing
...
function principalCurvePT(cellIdxs, root) {{
  ...
  if (rootValid) {{
    ...
    for (let a = 0; a < n; a++) lam[a] = X[a][0];   // UMAP1
  }}
  const normalize = arr => {{
    let lo = Infinity, hi = -Infinity;
...
document.getElementById('qc-pt').addEventListener('click', () => {{
  ...
  // active cells → viridis. Unrooted here (UMAP1-seeded); no root picker in this app.
  const active = [];
