#!/usr/bin/env python
"""Heatmap explorer: SVD biplot + in-browser SVD recompute PLUS an enhanced
cell x gene expression heatmap (shown-gene rows; rooted-pseudotime, cell-type+
rooted, and sort-by-gene cell-ordering modes). Cloned from
build_svd_recompute_app_3d.py.
...
OUT         = os.path.join(base.ROOT, 'notebooks',
                           f'{SLUG}_heatmap_explorer_3d.html')
NPC         = 3
...
<head>
<meta charset="utf-8">
<title>{GROUP_NAME} heatmap explorer</title>
<style>
...
.heatmap-controls .order-btn, .heatmap-controls .mode-btn {{ padding: 1px 7px; font-size: 11px;
                                 border: 1px solid #bbb; background: #f6f6f6;
                                 border-radius: 3px; cursor: pointer; }}
.heatmap-controls .order-btn:hover, .heatmap-controls .mode-btn:hover {{ background: #eee; }}
.heatmap-controls .order-btn.active, .heatmap-controls .mode-btn.active {{ background: #1f77b4; color: white;
                                        border-color: #1f77b4; cursor: default; }}
.heatmap-controls #pick-root-btn.armed {{ background: #e8590c; color: white; border-color: #e8590c; }}
...
{base.HOME_LINK_HTML}
<div class="wrap">
<h2>{base.cohort_title(GROUP_NAME)} — heatmap explorer</h2>
<div class="hint">
...
<div class="viz-stack">
  <div class="heatmap-row">
    <div class="heatmap-caption">
      <span id="heatmap-caption-text">shown genes z-expression — genes by argmax of smoothed expression.</span> <span id="heatmap-info" style="color:#888;"></span>
      ...
      <span class="heatmap-controls">
        <button id="pick-root-btn" class="order-btn" title="Click, then click a cell on the 3D cell plot to set the pseudotime root.">Pick root cell</button>
        <button id="clear-root-btn" class="order-btn" title="Clear the picked root cell.">clear root</button>
        <span class="label">sort by gene</span>
        <input id="heatmap-gene-input" list="gene-datalist" placeholder="e.g. Cnr1" autocomplete="off"
               style="font-size:11px; padding:1px 4px; width:90px;">
      </span>
    </div>
...
// Flipped true after the first successful renderHeatmap so applyGeneFilter can
// safely trigger heatmap re-renders on gene-filter changes (var → hoisted).
...
function applyGeneFilter() {{
  ...
  // The heatmap rows are the SHOWN-gene set, so re-render when the filter
  // changes. Guard so this no-ops harmlessly before the first heatmap render
  // (renderHeatmap is hoisted; heatmapReady flips true after the initial draw).
  if (heatmapReady && typeof renderHeatmap === 'function') {{
    try {{ renderHeatmap(); }} catch (e) {{ /* heatmap not ready yet */ }}
  }}
}}
...
let heatmapActiveIdx = null;            // length-m_active cell indices in current order
let heatmapRowGenes  = null;            // global gene index for each heatmap row, in row order
let heatmapGeneToRow = null;            // length-n_genes: row index in heatmap, or -1
...
// Heatmap sorting mode: 'pc' (existing PC-axis order) | 'rooted' (distance from a
// picked root cell) | 'celltype_rooted' (group by type, sort within by rooted dist)
// | 'gene' (sort cells by a gene's expression; rows by correlation to that gene).
let heatmapMode = 'pc';
let rootCell = null;            // global cell index of the picked pseudotime root, or null
let rootPickArmed = false;      // true while waiting for a cell click to set the root
let heatmapSortGene = -1;       // global gene index chosen for 'gene' mode, or -1

// Rooted distance: Euclidean distance in cell_score space (over available PCs)
// from cell i to the current root. sqrt(Σ_k (score[i][k]-score[root][k])^2).
function rootedDist(i) {{
  const a = cell_score[i], b = cell_score[rootCell];
  const K = Math.min(a.length, b.length);
  let s = 0;
  for (let k = 0; k < K; k++) {{ const d = a[k] - b[k]; s += d * d; }}
  return Math.sqrt(s);
}}
...
function renderHeatmap() {{
  ...
  // 1. Active cells ordered by the chosen mode (optionally grouped by subtype first)
  let activeIdx = [];
  ...
  if (heatmapMode === 'rooted' || heatmapMode === 'celltype_rooted') {{
    ...
    if (heatmapSortGene >= 0) {{
      ...
      caption += ' — cells by ' + gene_name[heatmapSortGene] + ' expression';
    }} else {{
      sortKey = (i => cell_score[i][0]);
  ...
  const groupByType = heatmapGroupByType || heatmapMode === 'celltype_rooted';
  if (groupByType) {{
  ...
  // ROW GENE SET: only the currently-shown genes (gene-set ∩ mean/std ∩
  // recoverability filter). Fall back to panel_idx when <2 shown genes.
  let rowGenes = (typeof visibleGeneIdx === 'function') ? visibleGeneIdx() : panel_idx.slice();
  if (!rowGenes || rowGenes.length < 2) rowGenes = panel_idx.slice();
  const nP = rowGenes.length;
  ...
  // 3+4. Order the gene ROWS.
  const win = Math.max(5, Math.min(m, Math.floor(m / 120) | 0));
  const halfW = win >> 1;
  const order = [];
  for (let p = 0; p < nP; p++) order.push(p);
  if (heatmapMode === 'gene' && heatmapSortGene >= 0) {{
    // Order rows by Pearson correlation to the chosen gene over ACTIVE cells,
    // DESCENDING (most positively-correlated shown genes at the top).
    // The chosen gene is z-scored over active cells exactly like the rows.
    let gs = 0, gss = 0;
    const gvals = new Float64Array(m);
    for (let ii = 0; ii < m; ii++) {{
      const v = readVal(activeIdx[ii], heatmapSortGene);
      gvals[ii] = v; gs += v; gss += v*v;
    }}
    const gmean = gs / m;
    const gstd = Math.sqrt(Math.max(gss/m - gmean*gmean, 1e-12));
    const gz = new Float64Array(m);
    for (let ii = 0; ii < m; ii++) gz[ii] = (gvals[ii] - gmean) / gstd;
    const corrOf = new Float64Array(nP);
    for (let p = 0; p < nP; p++) {{
      const rrow = Zg[p];        // already z-scored (mean 0, unit var) over active cells
      let dot = 0;
      for (let ii = 0; ii < m; ii++) dot += rrow[ii] * gz[ii];
      corrOf[p] = dot / m;       // Pearson r (both standardized)
    }}
    order.sort((a, b) => corrOf[b] - corrOf[a]);
  }} else {{
    // Default: smooth each gene row, sort by argmax (early → late).
  ...
  // Caption: report the shown-gene row count + the active mode hint.
  const capEl = document.getElementById('heatmap-caption-text');
  if (capEl) capEl.textContent = caption + '.';
  const rootHint = (rootCell !== null) ? '  root: cell #' + rootCell : '';
  heatInfo.textContent = '— shown genes (' + nP + ') × ' + m + ' cells (z-clipped ±'
    + Z_CLIP + ', smoothing window=' + win + ')' + rootHint;
  ...
  // Row bookkeeping over the SHOWN-gene set: row p (top→bottom) shows
  // rowGenes[order[p]]; heatmapGeneToRow maps a global gene index → its row, or -1.
  heatmapRowGenes = new Int32Array(nP);
  heatmapGeneToRow = new Int32Array(gene_name.length).fill(-1);
  for (let rowPos = 0; rowPos < nP; rowPos++) {{
    const g = rowGenes[order[rowPos]];
    heatmapRowGenes[rowPos] = g;
    heatmapGeneToRow[g] = rowPos;
  }}
  heatmapReady = true;
...
// Thin white outline around the gene's row in the heatmap (only for shown genes).
function drawHeatmapOverlay(j) {{
  clearHeatmapOverlay();
  if (!heatmapActiveIdx || heatmapActiveIdx.length < 4) return;
  if (j < 0 || j >= gene_name.length) return;
  if (!heatmapGeneToRow) return;
  const rowPos = heatmapGeneToRow[j];
  if (rowPos < 0) return;               // not a shown-gene row — nothing to outline
  const ctx = heatOverlay.getContext('2d');
  const W = heatOverlay.width, H = heatOverlay.height;
  const dpr = window.devicePixelRatio || 1;
  const nP = heatmapRowGenes.length;
  const rowH = H / nP;
...
// Wire the PC/pseudotime order buttons (scoped to those carrying data-axis so
// the root-pick/clear buttons, which share the .order-btn style, are excluded).
document.querySelectorAll('.order-btn[data-axis]').forEach(btn => {{
  btn.addEventListener('click', () => {{
    heatmapOrderAxis = parseInt(btn.dataset.axis) || 0;
    document.querySelectorAll('.order-btn[data-axis]').forEach(b =>
      b.classList.toggle('active', b === btn));
    // Picking a PC axis implies PC-ordering mode.
    setHeatmapMode('pc');
    renderHeatmap();
  }});
}});
...
document.querySelectorAll('.mode-btn').forEach(btn => {{
  btn.addEventListener('click', () => {{
    setHeatmapMode(btn.dataset.mode);
    renderHeatmap();
  }});
}});

// ---- Rooted-pseudotime: pick a root cell by clicking the cell plot --------
const pickRootBtn  = document.getElementById('pick-root-btn');
const clearRootBtn = document.getElementById('clear-root-btn');
if (pickRootBtn) {{
  pickRootBtn.addEventListener('click', () => {{
    rootPickArmed = !rootPickArmed;
    pickRootBtn.classList.toggle('armed', rootPickArmed);
    pickRootBtn.textContent = rootPickArmed ? 'Click a cell…' : 'Pick root cell';
  }});
}}
if (clearRootBtn) {{
...
// A click on the cell plot's points trace sets the root (global cell index).
cellPlot.on('plotly_click', function(data) {{
  if (!rootPickArmed) return;
  const pt = data.points[0];
  if (!pt || pt.curveNumber !== POINTS_TRACE) return;
  rootCell = pt.pointNumber;            // global cell index
  rootPickArmed = false;
  if (pickRootBtn) {{ pickRootBtn.classList.remove('armed'); pickRootBtn.textContent = 'Pick root cell'; }}
  // Picking a root implies a rooted mode; default to plain rooted pseudotime
  // unless the user is already grouping by cell type.
  if (heatmapMode !== 'rooted' && heatmapMode !== 'celltype_rooted') setHeatmapMode('rooted');
  renderHeatmap();
}});

// ---- Sort-by-gene input ---------------------------------------------------
const heatmapGeneInput = document.getElementById('heatmap-gene-input');
if (heatmapGeneInput) {{
  const applyHeatmapGene = () => {{
    const q = heatmapGeneInput.value.trim().toLowerCase();
    if (!q) {{ heatmapSortGene = -1; if (heatmapMode === 'gene') renderHeatmap(); return; }}
    let j = gene_name.findIndex(g => g.toLowerCase() === q);
    if (j < 0) j = gene_name.findIndex(g => g.toLowerCase().startsWith(q));
    if (j < 0) return;                  // no match — leave current state
    heatmapSortGene = j;
    setHeatmapMode('gene');
    renderHeatmap();
  }};
  heatmapGeneInput.addEventListener('change', applyHeatmapGene);
  heatmapGeneInput.addEventListener('keydown', e => {{ if (e.key === 'Enter') applyHeatmapGene(); }});
}}
...
function updateCellTypeOrderUI() {{
  ...
  if (!row) return;
  const grouping = heatmapGroupByType || heatmapMode === 'celltype_rooted';
  if (!grouping) {{ row.style.display = 'none'; return; }}
  row.style.display = '';
