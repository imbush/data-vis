...
function saveFigure(dark) {{
  ...
  if (is3d) {{
    ...
    if (typeof TET_V !== 'undefined' && typeof K_ARCH !== 'undefined') {{
      // NMF: draw the archetype simplex (triangle/tetrahedron) outline + vertex
      // labels instead of an axis grid — matches the interactive plot.
      const V = (typeof activeSimplexV === 'function') ? activeSimplexV() : TET_V;
      const KA = V.length, POLE = 1.22;
      const ex=[], ey=[], ez=[];
      for (let a=0; a<KA; a++) for (let b=a+1; b<KA; b++) {{
        ex.push(V[a][0], V[b][0], null); ey.push(V[a][1], V[b][1], null); ez.push(V[a][2], V[b][2], null);
      }}
      traces.push({{type:'scatter3d', mode:'lines', x:ex, y:ey, z:ez,
        line:{{color: dark ? '#8a93a3' : '#b8b8b8', width:2}}, hoverinfo:'skip', showlegend:false}});
      const vx=[], vy=[], vz=[], vt=[];
      for (let k=0; k<KA; k++) {{
        vx.push(V[k][0]*POLE); vy.push(V[k][1]*POLE); vz.push(V[k][2]*POLE);
        vt.push((typeof pole_top !== 'undefined' && pole_top[k]) ? pole_top[k] : ('A'+(k+1)));
      }}
      traces.push({{type:'scatter3d', mode:'markers+text', x:vx, y:vy, z:vz, text:vt,
        textposition:'top center', textfont:{{color:FG, size:11}}, marker:{{size:3, color:FG}},
        hoverinfo:'skip', showlegend:false}});
      const hid = {{visible:false}};
      layout.scene = {{ camera:cam, domain:{{x:[0,1], y:[0.15,0.96]}}, aspectmode:'cube', xaxis:hid, yaxis:hid, zaxis:hid }};
    }} else {{
...
function runSearch() {{
  ...
  // colour key + saved-figure state for the searched gene (was missing)
...
// Embedding coords come from cell_score[i] (first 3 NMF components; missing
...
function principalCurvePT(cellIdxs, root) {{
  ...
  if (rootValid) {{
    ...
    for (let a = 0; a < n; a++) lam[a] = X[a][0];   // first component
  }}
  const normalize = arr => {{
    let lo = Infinity, hi = -Infinity;
...
document.getElementById('qc-pt').addEventListener('click', () => {{
  ...
  // active cells → viridis. Unrooted here (first-component-seeded); no root picker.
  const active = [];
