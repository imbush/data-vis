...
<div class="ctrl-box">
  ...
  <div class="controls-row">
    ...
    <button id="recompute-all-btn" title="Refit the diffusion embedding on every gene currently shown in the gene filter (the active gene set ∩ mean/std/ribo sliders), using only the checked-subtype cells. Slower than the panel fit.">Replot with shown genes</button>
...
// Embedding coords come from cell_dc[i] (length-3 [DC1,DC2,DC3]; missing dims
// treated as 0). Uses NC=min(n,200) moving-average curve anchors for speed.
...
function principalCurvePT(cellIdxs, root) {{
  ...
  const coord = (idx, k) => {{ const s = cell_dc[idx]; return (s && s.length > k) ? s[k] : 0; }};
  ...
  let lam = new Array(n);
  if (rootValid) {{
    // Euclidean distance in cell_dc from the root cell.
    ...
    for (let a = 0; a < n; a++) lam[a] = X[a][0];   // DC1
  }}
  const normalize = arr => {{
    let lo = Infinity, hi = -Infinity;
...
// the "Replot with shown genes" button so the embedding is fit on whatever genes
